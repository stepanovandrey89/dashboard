(function(){const e=document.createElement("link").relList;if(e&&e.supports&&e.supports("modulepreload"))return;for(const i of document.querySelectorAll('link[rel="modulepreload"]'))s(i);new MutationObserver(i=>{for(const n of i)if(n.type==="childList")for(const a of n.addedNodes)a.tagName==="LINK"&&a.rel==="modulepreload"&&s(a)}).observe(document,{childList:!0,subtree:!0});function t(i){const n={};return i.integrity&&(n.integrity=i.integrity),i.referrerPolicy&&(n.referrerPolicy=i.referrerPolicy),i.crossOrigin==="use-credentials"?n.credentials="include":i.crossOrigin==="anonymous"?n.credentials="omit":n.credentials="same-origin",n}function s(i){if(i.ep)return;i.ep=!0;const n=t(i);fetch(i.href,n)}})();class L{constructor(){this.farms=[],this.rigIds=[],this.offlineThreshold=15*60*1e3,this.selectedFarm=null,this.coreTemperatureThreshold=85,this.memoryTemperatureThreshold=95}setOfflineThreshold(e){this.offlineThreshold=e}setCoreTemperatureThreshold(e){this.coreTemperatureThreshold=e}setMemoryTemperatureThreshold(e){this.memoryTemperatureThreshold=e}async loadFarms(){try{const e=await fetch("data/rig_ids.json");this.rigIds=await e.json();const t=this.rigIds.map(i=>this.loadFarmData(i)),s=await Promise.all(t);this.farms=s.filter(i=>i!==null),this.renderFarmsTable()}catch(e){throw console.error("Ошибка загрузки ферм:",e),e}}async loadFarmData(e){try{const t=await fetch(`data/hello_${e.id}.json`);if(!t.ok)throw new Error(`HTTP ${t.status}`);const s=await t.json();let i=null,n=!1,a=new Date;try{const o=await fetch(`data/stats_${e.id}.json`);if(o.ok){i=await o.json();const r=o.headers.get("last-modified");r?a=new Date(r):a=new Date(Date.now()-5*60*1e3),n=Date.now()-a.getTime()<=this.offlineThreshold}}catch(o){console.warn(`Нет stats данных для фермы ${e.id}:`,o.message),n=!1}return this.processFarmData(e,s,i,n,a)}catch(t){return console.error(`Ошибка загрузки данных фермы ${e.id}:`,t),null}}processFarmData(e,t,s,i,n){const a=t.params.gpu.filter(d=>d.brand!=="intel"&&d.brand!=="cpu"),o=i?this.calculateHashrate(s):0,r=i?this.calculatePower(s):0,c=this.calculateUptime(t.params.boot_time),l=i?this.getSolutions(s):0;return{id:e.id,name:e.name,isOnline:i,lastModified:n,gpuCount:a.length,gpus:a,hashrate:o,power:r,uptime:c,solutions:l,temperature:i?this.getAverageTemperature(s):0,helloData:t,statsData:s,notificationShown:!1}}calculateHashrate(e){var i,n;if(!e||!((i=e.params.miner_stats)!=null&&i.hs))return 0;let t=e.params.miner_stats.hs.reduce((a,o)=>a+(o||0),0);const s=((n=e.params.miner)==null?void 0:n.toLowerCase())||"";return s.includes("rigel")?t*=1024:(s.includes("custom")&&e.params.miner_stats.hs_units==="hs"||e.params.miner_stats.hs_units==="hs")&&(t=t),t}calculatePower(e){return e?(e.params.power||[]).filter(s=>s!==0).reduce((s,i)=>s+i,0):0}calculateUptime(e){return Math.floor((Date.now()-e*1e3)/1e3)}getAverageTemperature(e){var i;if(!e)return 0;const s=(((i=e.params.miner_stats)==null?void 0:i.temp)||[]).filter(n=>n>0);return s.length>0?Math.round(s.reduce((n,a)=>n+a,0)/s.length):0}getSolutions(e){var t,s;return e&&((s=(t=e.params.miner_stats)==null?void 0:t.ar)==null?void 0:s[0])||0}renderFarmsTable(e="farms-tbody-overview",t=!0){const s=document.getElementById(e);s&&(s.innerHTML="",this.farms.forEach(i=>{const n=this.createFarmRow(i,t);s.appendChild(n)}))}createFarmRow(e,t=!0){const s=document.createElement("tr");s.dataset.farmId=e.id;const i=e.isOnline?"online":"offline",n=e.isOnline?"Online":"Offline",a=e.gpus.map(r=>`<span class="gpu-indicator ${r.brand}" title="${r.name}"></span>`).join("");let o="";return e.isOnline&&e.hashrate>0&&(e.hashrate>1e3?o=(e.hashrate/1e3).toFixed(2)+" MH/s":o=e.hashrate.toFixed(2)+" H/s"),s.innerHTML=`
        ...
        `,typeof lucide<"u"&&lucide.createIcons())}formatMarketCap(e){return e?e>=1e12?(e/1e12).toFixed(2)+"T":e>=1e9?(e/1e9).toFixed(2)+"B":e>=1e6?(e/1e6).toFixed(2)+"M":e>=1e3?(e/1e3).toFixed(2)+"K":e.toLocaleString():"N/A"}}class C{constructor(){this.farmManager=new L,this.notificationManager=new B,this.themeManager=new D,this.ratesManager=new N,this.currentSection="overview",this.updateInterval=1e4,this.updateTimer=null,this.offlineThreshold=15*60*1e3,this.timeoutOptions=[15,30,60,120,240,480,720,1440],this.currentTimeoutIndex=0,this.allPayments=[],this.poolData=null,this.init()}async init(){try{typeof lucide<"u"&&lucide.createIcons(),this.setupNavigation(),this.setupMobileMenu(),this.setupThemeToggle(),this.setupOfflineTimeoutControl(),this.setupFarmDetails(),this.setupRates(),this.setupAlertSettings(),await this.loadInitialData(),this.startAutoUpdate(),this.notificationManager.show("Дашборд успешно загружен","success")}catch(e){console.error("Ошибка инициализации:",e),this.notificationManager.show("Ошибка загрузки дашборда","error")}}setupMobileMenu(){const e=document.getElementById("mobile-menu-toggle"),t=document.getElementById("sidebar"),s=document.getElementById("sidebar-overlay");e&&t&&s&&(e.addEventListener("click",()=>{t.classList.toggle("open"),s.classList.toggle("active")}),s.addEventListener("click",()=>{t.classList.remove("open"),s.classList.remove("active")}),document.querySelectorAll(".nav-item").forEach(i=>{i.addEventListener("click",()=>{window.innerWidth<=1024&&(t.classList.remove("open"),s.classList.remove("active"))})}))}setupNavigation(){document.querySelectorAll(".nav-item").forEach(t=>{t.addEventListener("click",s=>{s.preventDefault();const i=t.dataset.section;this.switchSection(i)})})}switchSection(e){document.querySelectorAll(".nav-item").forEach(t=>{t.classList.remove("active")}),document.querySelector(`[data-section="${e}"]`).classList.add("active"),document.querySelectorAll(".content-section").forEach(t=>{t.classList.remove("active")}),document.getElementById(`${e}-section`).classList.add("active"),this.updatePageTitle(e),this.loadSectionData(e),this.currentSection=e}updatePageTitle(e){const t={overview:"Обзор майнинга",rates:"Курсы криптовалют",earnings:"Доходы",payments:"История выплат",settings:"Настройки"},s={overview:"Мониторинг ваших майнинг ферм в реальном времени",rates:"Актуальные курсы криптовалют",earnings:"Отслеживание последних вознаграждений",payments:"Полная история всех выплат",settings:"Настройка параметров дашборда"};document.querySelector(".page-title").textContent=t[e],document.querySelector(".page-subtitle").textContent=s[e]}setupThemeToggle(){const e=document.getElementById("theme-toggle");e.addEventListener("click",()=>{this.themeManager.toggle();const t=e.querySelector("i"),s=this.themeManager.getCurrentTheme();t.setAttribute("data-lucide",s==="dark"?"sun":"moon"),typeof lucide<"u"&&lucide.createIcons()})}setupOfflineTimeoutControl(){const e=document.getElementById("timeout-decrease"),t=document.getElementById("timeout-increase");document.getElementById("timeout-value"),this.loadGlobalSettings(),this.updateTimeoutDisplay(),this.updateOfflineThreshold(),e.addEventListener("click",()=>{this.currentTimeoutIndex>0&&(this.currentTimeoutIndex--,this.updateTimeoutDisplay(),this.updateOfflineThreshold(),this.saveGlobalSettings())}),t.addEventListener("click",()=>{this.currentTimeoutIndex<this.timeoutOptions.length-1&&(this.currentTimeoutIndex++,this.updateTimeoutDisplay(),this.updateOfflineThreshold(),this.saveGlobalSettings())})}loadGlobalSettings(){const e=localStorage.getItem("global-offline-timeout"),t=localStorage.getItem("global-core-temp-threshold"),s=localStorage.getItem("global-memory-temp-threshold");if(e){const i=this.timeoutOptions.indexOf(parseInt(e));i!==-1&&(this.currentTimeoutIndex=i)}t&&this.farmManager.setCoreTemperatureThreshold(parseInt(t)),s&&this.farmManager.setMemoryTemperatureThreshold(parseInt(s))}saveGlobalSettings(){const e=this.timeoutOptions[this.currentTimeoutIndex];localStorage.setItem("global-offline-timeout",e.toString()),localStorage.setItem("global-core-temp-threshold",this.farmManager.coreTemperatureThreshold.toString()),localStorage.setItem("global-memory-temp-threshold",this.farmManager.memoryTemperatureThreshold.toString())}updateTimeoutDisplay(){const e=document.getElementById("timeout-value"),t=document.getElementById("timeout-decrease"),s=document.getElementById("timeout-increase"),i=this.timeoutOptions[this.currentTimeoutIndex];i>=1440?e.textContent=`${i/1440} сут`:i>=60?e.textContent=`${i/60} ч`:e.textContent=`${i} мин`,t.disabled=this.currentTimeoutIndex===0,s.disabled=this.currentTimeoutIndex===this.timeoutOptions.length-1}updateOfflineThreshold(){const e=this.timeoutOptions[this.currentTimeoutIndex];this.offlineThreshold=e*60*1e3,this.farmManager.setOfflineThreshold(this.offlineThreshold),this.notificationManager.show(`Таймаут оффлайн установлен: ${document.getElementById("timeout-value").textContent}`,"info"),this.farmManager.updateFarms()}setupFarmDetails(){const e=document.getElementById("details-close");e&&e.addEventListener("click",()=>{this.farmManager.hideFarmDetails()})}setupRates(){const e=document.getElementById("refresh-rates");e&&e.addEventListener("click",()=>{this.ratesManager.loadRates(),this.notificationManager.show("Курсы обновлены","success")})}setupAlertSettings(){const e=document.getElementById("core-temp-threshold"),t=document.getElementById("memory-temp-threshold"),s=localStorage.getItem("global-core-temp-threshold"),i=localStorage.getItem("global-memory-temp-threshold");s&&(e.value=s,this.farmManager.setCoreTemperatureThreshold(parseInt(s))),i&&(t.value=i,this.farmManager.setMemoryTemperatureThreshold(parseInt(i))),e.addEventListener("change",n=>{const a=parseInt(n.target.value);this.farmManager.setCoreTemperatureThreshold(a),localStorage.setItem("global-core-temp-threshold",a.toString()),this.notificationManager.show(`Критическая температура ядра установлена: ${a}°C`,"info")}),t.addEventListener("change",n=>{const a=parseInt(n.target.value);this.farmManager.setMemoryTemperatureThreshold(a),localStorage.setItem("global-memory-temp-threshold",a.toString()),this.notificationManager.show(`Критическая температура памяти установлена: ${a}°C`,"info")})}async loadInitialData(){try{await this.farmManager.loadFarms(),await this.loadPoolData(),this.updateOverviewStats()}catch(e){throw console.error("Ошибка загрузки данных:",e),e}}async loadPoolData(){try{const e=await fetch("data/pool.json");this.poolData=await e.json(),document.getElementById("coin-symbol").textContent=this.poolData.pool.coinSymbol,document.getElementById("coin-price").textContent=`$${parseFloat(this.poolData.pool.coinPriceUsd).toFixed(2)}`,document.getElementById("blocks-found").textContent=this.poolData.miner.totalBlocksFound,document.getElementById("coin-symbol-earnings").textContent=this.poolData.pool.coinSymbol,document.getElementById("coin-price-earnings").textContent=`$${parseFloat(this.poolData.pool.coinPriceUsd).toFixed(2)}`,document.getElementById("coin-symbol-payments").textContent=this.poolData.pool.coinSymbol,document.getElementById("coin-price-payments").textContent=`$${parseFloat(this.poolData.pool.coinPriceUsd).toFixed(2)}`,this.updateRewardsTable(this.poolData),this.allPayments=this.poolData.miner.payments,this.updateAllPaymentsTable()}catch(e){console.error("Ошибка загрузки данных пула:",e)}}updateRewardsTable(e){const t=document.getElementById("rewards-tbody");if(!t)return;const s=[{label:"Последние 60 минут",index:0},{label:"Последние 12 часов",index:1},{label:"Последние 24 часа",index:2},{label:"Сегодня",index:3},{label:"Вчера",index:4},{label:"Предыдущие 7 дней",index:5},{label:"Предыдущие 30 дней",index:6}];t.innerHTML="",s.forEach(i=>{var c;const n=((c=e.miner.sumrewards[i.index])==null?void 0:c.reward)||0,a=(n/1e8).toFixed(5),o=(n/1e8*parseFloat(e.pool.coinPriceUsd)).toFixed(2),r=document.createElement("tr");r.innerHTML=`
                <td>${i.label}</td>
                <td>${a}</td>
                <td>$${o}</td>
            `,t.appendChild(r)})}updateAllPaymentsTable(){const e=document.getElementById("all-payments-tbody"),t=document.getElementById("payments-total");if(!e||!this.poolData)return;e.innerHTML="",this.allPayments.forEach(n=>{const a=document.createElement("tr"),o=new Date(n.timestamp*1e3).toLocaleString("ru-RU"),r=(n.amount/1e8).toFixed(5);a.innerHTML=`
                <td>${o}</td>
        ...
        `,typeof lucide<"u"&&lucide.createIcons()}updateOverviewStats(){const e=this.farmManager.getFarms(),t=e.filter(l=>l.isOnline),s=e.filter(l=>!l.isOnline),i=t.reduce((l,d)=>l+d.gpuCount,0),n=t.reduce((l,d)=>l+d.hashrate,0),a=t.reduce((l,d)=>l+d.power,0);document.getElementById("farms-count").textContent=t.length;const o=document.getElementById("offline-farms-count");s.length>0?(o.textContent=`${s.length} неактивных`,o.style.display="block"):o.style.display="none",document.getElementById("gpu-count").textContent=i;const r=document.getElementById("hashrate");n>1e3?(r.textContent=(n/1e3).toFixed(2),r.setAttribute("data-unit","MH/s")):(r.textContent=n.toFixed(2),r.setAttribute("data-unit","H/s"));const c=document.getElementById("power-consumption");c.textContent=(a/1e3).toFixed(2),c.setAttribute("data-unit","kW"),s.length>0&&s.forEach(l=>{l.notificationShown||(this.notificationManager.show(`Ферма ${l.name} не в сети`,"warning"),l.notificationShown=!0)}),this.farmManager.renderFarmsTable("farms-tbody-overview",!0)}async loadSectionData(e){switch(e){case"overview":this.updateOverviewStats();break;case"rates":await this.ratesManager.loadRates();break;case"earnings":this.poolData&&this.updateRewardsTable(this.poolData);break;case"payments":this.updateAllPaymentsTable();break}}startAutoUpdate(){this.updateTimer=setInterval(async()=>{try{await this.farmManager.updateFarms(),this.updateOverviewStats()}catch(e){console.error("Ошибка автообновления:",e)}},this.updateInterval)}stopAutoUpdate(){this.updateTimer&&(clearInterval(this.updateTimer),this.updateTimer=null)}}document.addEventListener("DOMContentLoaded",()=>{window.dashboard=new C});window.addEventListener("error",m=>{console.error("Глобальная ошибка:",m.error)});window.addEventListener("unhandledrejection",m=>{console.error("Необработанный промис:",m.reason)});
